...
if laravel_is_installed; then
    ...
    mkdir -p "$APP_BASE_DIR/storage/logs" || true
    mkdir -p "$APP_BASE_DIR/storage/framework/cache" || true
    mkdir -p "$APP_BASE_DIR/storage/framework/views" || true
    mkdir -p "$APP_BASE_DIR/storage/framework/sessions" || true
    mkdir -p "$APP_BASE_DIR/storage/app/private" || true
    mkdir -p "$APP_BASE_DIR/storage/app/public" || true
