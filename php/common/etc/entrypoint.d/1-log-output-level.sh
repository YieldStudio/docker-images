...
set_php_ini (){
    ...
    php_fpm_debug_conf_file="/usr/local/etc/php-fpm.d/zzz-docker-php-fpm-debug.conf"
...
set_fpm_log_level (){
    ...
    if [ "$fpm_log_level" = "debug" ]; then 
        echo "access.log = /proc/self/fd/2" >> /usr/local/etc/php-fpm.d/zzz-docker-php-fpm-debug.conf
        echo "access.format = \"fpm: %R - %u %t \"%m %r%Q%q\" %s duration=%{milliseconds}dms memory=%Mk cpu=%C%% pid=%p script=%f\"" >> /usr/local/etc/php-fpm.d/zzz-docker-php-fpm-debug.conf
